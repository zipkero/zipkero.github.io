...
- Docker는 실제 가상화 방식과 달리 게스트 운영체제를 설치하는 대신 호스트 운영체제를 기반으로 이미지화한다.
프로세스를 일부 격리하지만 동일한 명령어가 있으면 컨테이너에서 실행하게 되며, 호스트 운영체제에서 실행되므로 호스트 운영체제의 프로세스 공간을 공유한다.
...
- Dockerfile을 기반으로 이미지를 생성하는 방법은 아래와 같다.
