---
date: "2023-06-22"
title: "Private ECS 세팅"
tags: ['aws', 'ecs', 'ecr']
summary: "AWS ECS 클러스터를 public에서 private 환경으로 변경하면서 나온 에러들을 정리"
---

개발 과정에서 빠른 확인을 위해 ECS 클러스터에 Public Subnet을 지정하였고, 생성과정과 API테스트 시 문제점이 발견되지 않아 Private Subnet으로 변경하는 과정을 정리함.

1. Private Subnet 으로 클러스터 재 생성

ECR에 저장된 이미지 가져오지 못하는 상황이 발생!
...
구글링을 통해 VPC 엔드포인트를 추가해야 VPC와 각 서비스간의 연결이 가능하다고 하여
